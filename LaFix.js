...
function getPage(i, pages) {
  if (i < pages) {
    changeHeader(
      filter_41 + " " + (i + 1) + "/" + pages + " <img src='graphic/throbber.gif' height='24' width='24'></img>"
    );
    var url =
      link[0] +
    ...
    success: function (data) {
      ...
      if (typeof history !== "undefined" && typeof history.pushState === "function") {
        history.pushState(
          ...
          "https://" + window.top.location.host + game_data.link_base_pure + "am_farm"
        );
      }
...
function applyFilters() {
  ...
  if (window.top.$('*:contains("Bot Protection")').length) {
    window.top
      .$("html, body")
      .animate({ scrollTop: window.top.$('*:contains("Bot Protection")').offset().top - topContainer }, 500);
...
function exportProfile() {
  ...
  if (profileName == profile_10) {
    ...
    var profileSettings = prompt(
      dialog_08,
      dialog_09A + "" + profileName + "" + dialog_09B + "" + profileName + "," + settings + "" + dialog_09C
    );
  }
}
